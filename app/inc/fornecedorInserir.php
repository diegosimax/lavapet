<?php
    ...
    $cnpj = trim(utf8_decode($_POST["cnpj"]));    
    ...
    if($erro == 0){
        if(salvarFornecedor($imagemUpload, $nome, $email, $telefone1, 
                         $telefone2, $cep, $endereco, $numero,
                         $complemento, $bairro, $cidade, $cnpj, $observacao)){
            salvaHistorico(utf8_decode("Incluído fornecedor ").$nome, utf8_decode("Fornecedores | Inserir"), $id_usuario);
            $_SESSION["MSG_SUCESSO"] = "Fornecedor ".utf8_encode($nome)." incluído com sucesso :-)";
            echo "<script type='text/javascript'> window.location = '../fornecedores/' </script>";
        }else{
             echo "<div class='alert alert-danger alert-dismissible'>
                 <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
                 <h4><i class='icon fa fa-ban'></i> Alerta!</h4>
                 Desculpe mas houve um erro ao inserir o fornecedor.
                 </div>";
        }       
    }

    function salvarFornecedor($imagemUpload, $nome, $email, $telefone1, 
                           $telefone2, $cep, $endereco, $numero,
                           $complemento, $bairro, $cidade, $cnpj, $observacao){        
        ...
        try{
            ...
            $stmt = $conn->prepare('INSERT INTO fornecedores 
                                    (nome, email, telefone1, 
                                     ...
                                     cidade, cnpj, observacao, criado_em) 
                                    VALUES 
                                    (:nome, :email, :telefone1, 
                                     :telefone2, :cep, :endereco, 
                                     :numero, :complemento, :bairro, 
            ...
            $stmt->execute(array('nome' => $nome, 'email' => $email, 
                                 ...
                                 'cnpj' => $cnpj, 'observacao' => $observacao, 'criado_em' => $criado_em));
            ...
            if(!empty($imagemUpload)){
                ...
                $stmt->execute(array('imagem' => $imagemUpload, 'id_fornecedor' => $ultimoId));
